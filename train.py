...
def make_tone_train(tones, traindur=10.0, tonedur=1.0, amplitude=1.0, fadedur=0.5):
    ...
    Args:
        tones (recordings.Recording): source recordings.
...
def make_simple_train(coords, sounds, length=10, graindur=(.5, 2), maxdist=60, train=None, pow2len=False, forcesource=-1):
    ...
    Args:
        coords (numpy.ndarray): barycentric coordinates of the mix in the uniform polygon formed by the sources as 
            edges. 
        sounds (list): list of instances of recordings.Recording, one per source edge.
...
class GrainTrain:
    ...
    Properties:
        ...
        sources (list): list of instances of recordings.Recording. These are the sources from which each grain obtains
            its samples.
    ...
    def set_sources(self, sources):
        ...
        Args:
            sources (list): list of instances of recordings.Recording.
        """
    ...
    def mixdown(self):
        """
        Set self.sound to an instance of recordings.Recording, the final mixed sound file for the grain train.
        ...
        Returns:
            self.sound, an instance of recordings.Recording, the final mixed recording.
        """
        
        self.sound = rec.Recording()
    ...
    def plot_train(self, framesize=512, hopsize=256, cmap=mpl.cm.gist_rainbow, npoints=3):
        ...
        for i, g in enumerate(self.grains):
            ...
            oldpos = g.outpos + len(g.env)
            
            ymin = 0 if not layer else 0.5
            xmin, xmax = g.outpos / float(self.rate), (g.outpos + len(g.env)) / float(self.rate)
        ...
        # Tighten axes.
        tmin = np.amin([g.outpos for g in self.grains]) / float(self.rate)
        tmax = np.amax([g.outpos + len(g.env) for g in self.grains]) / float(self.rate)

        #print self.rate
        #print np.amin([g.outpos for g in self.grains]), np.amax([g.outpos + len(g.env) for g in self.grains])
        #print tmin, tmax
