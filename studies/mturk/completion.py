...
Create an image of Mechanical Turk study completion percentage for each input
coordinate.
...
def voronoi(x, y):
    ...
    minx = (np.amin(x) - np.mean(x)) * 10 + mx
    maxx = (np.amax(x) - np.mean(x)) * 10 + mx
    miny = (np.mean(y) - np.amin(y)) * 10 + my
    maxy = (np.amax(y) - np.mean(y)) * 10 + my
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Create an image of\
        Mechanical Turk study completion percentage for each input\
        coordinate.')
    parser.add_argument('mapping', metavar='txt', type=str, 
        default='mapping.txt', help='mapping CSV file. See makemapping.sh for\
        more information.')
    parser.add_argument('results', metavar='csv', type=str, 
        default='results.csv', help='results CSV from Mechanical Turk.')
    args = parser.parse_args()

    sounds = makedict(args.mapping)
    
    results = np.genfromtxt(
        args.results, delimiter='","', skip_header=1, dtype=str
    )
    
    results = np.genfromtxt(
        args.results, delimiter='","', usecols=range(results.shape[1]),
        dtype=str
    )
    ...
    for coord in sounds['naive']:
        ...
        for v in sounds['naive'][coord].values():
            ...
            if count:
                ...
                realism[coord] += np.sum([
                    int(r) for r in results[relevant, realismcol]
                ])
    ...
    colors = [
        '%.2f' % (realism['-'.join(['%.2f' % c for c in p])] / 9.) 
        for p in points
    ]
