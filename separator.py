...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Make use of envrepet.Recording to separate figure/ground from an environmental \
        sound file.')
    ...
    for filename in args.input:
        for i, (combo, outname) in enumerate(zip(batchdict.combos, outnames)):
            if args.verbosity > 0:
                print 'Config %d / %d' % (i + 1, len(batchdict.combos))
            ...
            if combo['type'] == 'repet':
                ...
                # Save plot PDFs.
                if combo['plot']:
                    plots = plot_repet(r, progress=progress['Plotting'], prefix=args.output, suffix=fnsuffix)
                    if args.verbosity > 1: 
                        print '    Plots saved:\n       ', '\n        '.join(plots)
            elif combo['type'] == 'mra':
                r = MRA(filename)
